namespace CombatCountdown.State
{
    ...
    public class RestRunningState : BaseState
    {
        private Brush brush;

        public RestRunningState(MainPageViewModel viewModel)
            :base(viewModel, State.RestRunning)
        {
        ...
        public override void Process()
        {
            ...
            if (elapsed >= GlobalData.GlobalData.RestLength)
            {
                //rest has finished
                base.stopwatch.Stop();
            ...
            var countdownTime = GlobalData.GlobalData.RestLength - elapsed;

            this.viewModel.CountdownTime = countdownTime;
            this.viewModel.ProgressValue =
                (this.stopwatch.Elapsed.TotalSeconds * 100) / GlobalData.GlobalData.RestLength.TotalSeconds;
        }
    }
}
